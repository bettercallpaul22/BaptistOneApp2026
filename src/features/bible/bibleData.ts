...
const bibleModuleLoaders: Record<BibleTranslationId, () => BibleModule> = {
  asv: () => require('../../assets/bible/EN-English/asv.json') as BibleModule,
  asvs: () => require('../../assets/bible/EN-English/asvs.json') as BibleModule,
  bishops: () => require('../../assets/bible/EN-English/bishops.json') as BibleModule,
  coverdale: () => require('../../assets/bible/EN-English/coverdale.json') as BibleModule,
  geneva: () => require('../../assets/bible/EN-English/geneva.json') as BibleModule,
  kjv: () => defaultBibleModule,
  kjv_strongs: () => require('../../assets/bible/EN-English/kjv_strongs.json') as BibleModule,
  net: () => require('../../assets/bible/EN-English/net.json') as BibleModule,
  tyndale: () => require('../../assets/bible/EN-English/tyndale.json') as BibleModule,
  web: () => require('../../assets/bible/EN-English/web.json') as BibleModule,
};
...
const getBibleModule = (id: BibleTranslationId) => {
  const loadedModule = loadedBibleModules[id];

  if (loadedModule) return loadedModule;

  const module = bibleModuleLoaders[id]();
  loadedBibleModules[id] = module;

  return module;
};

const getBibleIndex = (id: BibleTranslationId) => {
  const existingIndex = bibleIndexes.get(id);

  if (existingIndex) return existingIndex;

  const index = createBibleIndex(getBibleModule(id));
...
export const getChaptersForBook = (bookId: number) =>
  Array.from({ length: getChapterCount(bookId) }, (_, index) => index + 1);

export const getChapterVerses = ({ book, chapter }: BibleChapterRef, translationId: BibleTranslationId = defaultBibleTranslationId) =>
  getBibleIndex(translationId).versesByChapter.get(getChapterKey(book, chapter)) ?? [];
...
export const searchBibleVerses = (
  ...
  const verses = getBibleIndex(translationId).module.verses;
...
export const getBibleSearchSuggestions = (
  query: string,
  translationId: BibleTranslationId = defaultBibleTranslationId,
  limit = 12,
) => {
